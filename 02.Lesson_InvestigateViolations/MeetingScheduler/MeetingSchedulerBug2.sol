...
contract MeetingScheduler is IMeetingScheduler {
    ...
    function cancelMeeting(uint256 meetingId) external override {
        ...
        require(
            scheduledMeeting.status == MeetingStatus.PENDING, //meeting has to be pending
            "can't cancel a meeting if not started"
        );
        meetings[meetingId].status = MeetingStatus.CANCELLED; //Meeting.status should be cancelled
    }

    function endMeeting(uint256 meetingId) external override {
        ScheduledMeeting memory scheduledMeeting = meetings[meetingId];
        require(
            scheduledMeeting.status == MeetingStatus.STARTED, //metting has to be started
        ...
        meetings[meetingId].status = MeetingStatus.ENDED; //meeting.status should be ended
    }
